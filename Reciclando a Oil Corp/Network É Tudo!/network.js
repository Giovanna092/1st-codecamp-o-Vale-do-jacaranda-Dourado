...
function verificaCargo(index){
    if(info[index].cargo == "Diretoria" || info[index].cargo == "Liderança") {
        return true
    }
    return false
}

function verificaEmail(index){
    if(info[index].email.includes("@oilcorp.com.br")) {
        return true
    } 
    return false
}
...
for(let i=offset;i<=limit;i++){
    if(verificaCargo(i) && verificaEmail(i)){
        personFind += "Nome: " + info[i].nome + ", Cargo: " + info[i].cargo + ", Email: " + info[i].email + "\n"
